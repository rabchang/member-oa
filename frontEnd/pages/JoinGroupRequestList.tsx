...
export default function Home({ func }) {
  ...
  >(`/api/joinGroupRequest?status=${status}`);
  const [successMsg, $successMsg] = useState("");
  return (
    <Container component="main" maxWidth="md">
      <h1>User Join Group Requests</h1>
                  ...
                  >
                    ...
                    <TableCell align="right">
                      {row.userId === user?.data?.id ? (
                        ...
                        <div>
                          <Link href={"/form-progress-join-group?id=" + row.id}>
                          ...
                          {row.status === "pending" && (
                            <Button
                              onClick={async () => {
                                await fetch(
                                  `/api/joinGroupRequest/${row.id}/pass`,
                                  {
                                    method: "POST",
                                  }
                                );
                                mutate();
                                $successMsg(
                                  `Successfully passed Join Group Request from ${row.userName}. `
                                );
                              }}
                            >
                              Pass
                          ...
                          {row.status === "pending" && (
                            <Button
                              ...
                              onClick={async () => {
                                await fetch(
                                  `/api/joinGroupRequest/${row.id}/reject`,
                                  {
                                    method: "POST",
                                  }
                                );
...
Home.title = "Join Group Request List";
