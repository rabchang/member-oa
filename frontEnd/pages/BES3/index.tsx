...
function PeopleTable({}) {
  ...
  >(`/api/groups/show-last-month-join?group_id=1`);
  return (
...
function PeopleTableExit({}) {
  ...
  const { data, alternativeUI, mutate } = useAlternativeUI<
    JSONResultWrapper<UserInGroup[]>
  >(`/api/groups/show-last-month-exit?group_id=1`);
  return (
    <Container component="main" maxWidth="md">
...
const featuredData = [
  { name: "Authors List", href: "/BES3/users", description: "120 authors" },
  { name: "Funds List", href: "/BES3/funds", description: "40 funds" },
];

export default function Home({ func }) {
  const router = useRouter();
  const theme = useTheme();
  const { data: num1 } = useSWR<any>("/api/group-users-number?group_id=1", fetcher);
  const { data: num2 } = useSWR<any>("/api/group-insts-number?group_id=1", fetcher);
  ...
  useEffect(() => {
    ...
    if (data) {
      // loaded
      if (!data.data.find((x) => x.groupName === "BES3")) {
        router.push("/403");
      }
    }
  }, [data]);
  const dashBoardData = [
    {
      ...
      name: "people",
      value: num1?.data.usersNumber || 500,
    },
    {
      icon: MapsHomeWorkTwoToneIcon,
      name: "institutions",
      value: num2?.data.instsNumber || 30,
    },
    // { icon: PublicTwoToneIcon, name: "counties", value: 17 },
  ];
